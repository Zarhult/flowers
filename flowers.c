...
typedef enum flowerInfo {
  ...
  PETALS,
  STAMENS,
  FLWR_INFO_ENUM_NUM /* equal to the number of other enums */
} FlowerInfo;
...
typedef enum ovaryPos { INFERIOR, SUPERIOR } OvaryPos;

/* order of widgets in left/right flower widget arrays */
typedef enum sideWidget {
  BOX, /* contains all the other widgets of a side, including some outside of
          the array */
  COMBO_BOX,
...
GtkWidget *l_widgets[FLWR_WIDGET_NUM]; /* widgets for flower info */
GtkWidget *l_dummy_label; /* fills up space to push button to bottom */
GtkWidget *l_toggle_button;
gboolean l_shown = FALSE; /* whether or not left flower is shown */
SideData l_data = {LEFT};

GtkWidget *r_widgets[FLWR_WIDGET_NUM]; /* widgets for flower info */
GtkWidget *r_dummy_label; /* fills up space to push button to bottom */
GtkWidget *r_toggle_button;
gboolean r_shown = FALSE; /* whether or not right flower is shown */
SideData r_data = {RIGHT};

Flower flowers[FLWR_NUM];

static gchar *fam_to_string(FlowerFam f) {
  switch (f) {
  case FIVE_PARTED:
    return "5-parted";
    break;
...
static gchar *subfam_to_string(FlowerSubfam s) {
  switch (s) {
  case SOLANACEAE:
    return "Solanaceae";
    break;
...
static gchar *ovary_pos_to_string(OvaryPos p) {
  switch (p) {
  case INFERIOR:
    return "Inferior";
    break;
...
static gchar *subfam_to_nickname_string(FlowerSubfam s) {
  switch (s) {
  case SOLANACEAE:
    return "Nightshade";
    break;
...
static void display_flower(Side grid, FlowerSubfam f) {
  ...
  gchar *label_bufs[7] = { filepath_buf, petals_label_buf, sepals_label_buf, stamens_label_buf, ovary_label_buf, family_label_buf, nickname_label_buf };
  gint i;

  /* prepare info strings */
  g_strlcat(filepath_buf, flowers[f].info[FILENAME], sizeof(filepath_buf));
  g_strlcat(petals_label_buf, flowers[f].info[PETALS],
            sizeof(petals_label_buf));
  g_strlcat(sepals_label_buf, flowers[f].info[SEPALS],
            sizeof(sepals_label_buf));
  g_strlcat(stamens_label_buf, flowers[f].info[STAMENS],
            sizeof(stamens_label_buf));
  g_strlcat(ovary_label_buf, ovary_pos_to_string(flowers[f].ovaryPos),
            sizeof(family_label_buf));
  g_strlcat(family_label_buf, fam_to_string(flowers[f].family),
            sizeof(family_label_buf));
  g_strlcat(nickname_label_buf, subfam_to_nickname_string(f),
            sizeof(stamens_label_buf));
...
static void on_button_click(GtkButton *b, gpointer user_data) {
...
static void on_combo_box_change(GtkComboBox *combo_box, gpointer user_data) {
  FlowerSubfam chosen_flower = gtk_combo_box_get_active(
...
static void activate(GtkApplication *app, gpointer user_data) {
  ...
  flowers[ONAGRACEAE] =
      (Flower){{"onagraceae.png", "4", "4", "4 or 8"}, FOUR_PARTED, INFERIOR};
  ...
  flowers[POLYGONACEAE] =
      (Flower){{"polygonaceae.png", "3 tepals (rather than sepals)",
                "3 tepals (rather than petals)", "3 to 9"},
               THREE_PARTED,
               SUPERIOR};

  flowers[LILIACEAE] =
      (Flower){{"liliaceae.png", "6 colored tepals (rather than sepals)",
  ...
  for (i = 0; i < FLWR_NUM; i++) {
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(l_widgets[COMBO_BOX]),
                                   subfam_to_string(i));
  }
  ...
  for (i = 0; i < FLWR_NUM; i++) {
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(r_widgets[COMBO_BOX]),
                                   subfam_to_string(i));
  }
  ...
  g_signal_connect(GTK_BUTTON(l_toggle_button), "clicked",
                   G_CALLBACK(on_button_click), &l_data);
  g_signal_connect(GTK_BUTTON(r_toggle_button), "clicked",
                   G_CALLBACK(on_button_click), &r_data);
